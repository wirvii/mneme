...
Before a change, the coordinator runs `mneme config show workflow`. The
default `legacy` engine keeps the existing SDD lifecycle. With
`delivery_v2`, an SDD spec creates and approves the contract, then one WORK
executes it; SPEC and WORK are never parallel execution cycles for the same
change. Organic WORK skips the spec, while method (`standard` or `tdd`) and
verification depth remain independent choices.

The coordinator alone calls `work_begin`, `work_lock`, `work_amend`,
`work_complete`, and `work_resume`. Any role may read `work_get` and
`work_metrics`; an implementer may run factual `work_verify` without a state
transition. Only a `qa-tester` subagent may run `work_review`, and an
unresolved role fails closed. No subagent calls `spec_advance`.

Execution is bounded to one broad review, at most one correction, and one
targeted review before completion or escalation. Resume requires an explicit
human decision. Installation distributes these rules but never activates the
engine. `deep_quality = "always"` does not automatically run `quality verify`
in this beta. See `docs/delivery-workflow.md` for explicit activation,
host-wide configuration cost, reversal to `engine = "legacy"`, and the
evidence that remains local.
...
**`mneme sdd enable` distinguishes "publishing for the first time" from
"repository already activated, new machine" (SPEC-140).** When
`.mneme/sdd/.mneme-sdd` is already committed and `--apply` is not passed,
the convergence check above is skipped entirely — records this machine's
database does not know are the ordinary state of a fresh clone, not a
rejection — and the command reports what THIS machine is missing (hooks
not installed, records not yet imported) instead of the plan/four-warnings
preview. `--apply`'s behaviour, convergence refusal included, is
completely unchanged in every case; only the plain preview differs.
