...
## Quality gates and coverage ratchet (SPEC-115 S1 + SPEC-116 S2, EPIC-calidad)

A repository opts in by committing `.mneme/quality.toml` (this repo's own
copy is real, `schema_version = 2`, everything `enabled = false`). mneme
...
**Schema versions only ever WIDEN, never narrow** (D5/R2): a
`schema_version = 1` document (no `[coverage]`/`[ratchet]`) keeps parsing
exactly as it always did; `CurrentSchemaVersion` bumping to 2 did not
brick a single existing constitution.
