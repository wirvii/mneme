...
**§2a is a deliberate first third of a larger mechanism (BL-194).** It
covers ONLY the file format and the write path. The other two thirds —
reading those files back into the database (BL-201, needed before a
teammate's clone can pick anything up) and reconciling two machines that
independently created the same correlative (BL-202) — do not exist yet.
**A repository that enables §2a today gets a reviewable archive, not yet
...
guards against, and reading or reconciling such a file is BL-201/BL-202's
job, not this one's.
