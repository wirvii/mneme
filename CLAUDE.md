...
## Quality gates, coverage ratchet, and executable criteria (SPEC-115 S1 + SPEC-116 S2 + SPEC-117 S3, EPIC-calidad)

A repository opts in by committing `.mneme/quality.toml` (this repo's own
copy is real, `schema_version = 3`, everything `enabled = false`). mneme
runs the declared gates itself and binds the result to the exact commit —
...
keeps parsing exactly as it always did. `CurrentSchemaVersion` bumping to
2 (S2) and then to 3 (S3) did not brick a single existing constitution —
the second bump also CORRECTED the comparison from an equality check
(`!= 1 && != CurrentSchemaVersion`) to a genuine range
...
world, this repo's own included, before `Parse` ever read `enabled`.
