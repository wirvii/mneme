...
func TestSDDFile_ClassifyRecordPath(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "work record",
			path:     filepath.Join(repoRoot, ".mneme", "sdd", "work", "WORK-001.md"),
			wantKind: KindWork,
			wantID:   "WORK-001",
			wantOK:   true,
		},
		{
			name:     "work record requires digits",
			path:     filepath.Join(repoRoot, ".mneme", "sdd", "work", "WORK-x.md"),
			wantKind: KindIgnored,
			wantID:   "",
			wantOK:   false,
		},
		{
			name:     "empty work number is ignored",
			path:     filepath.Join(repoRoot, ".mneme", "sdd", "work", "WORK-.md"),
			wantKind: KindIgnored,
			wantID:   "",
			wantOK:   false,
		},
		{
			name:     "nested work record is ignored",
			path:     filepath.Join(repoRoot, ".mneme", "sdd", "work", "WORK-001", "record.md"),
			wantKind: KindIgnored,
			wantID:   "",
			wantOK:   false,
		},
