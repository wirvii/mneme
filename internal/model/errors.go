...
// ErrNotACriterion is returned by Sign when the targeted row's Kind does
// NOT start with "criterion" — Sign is an ATTESTATION, reserved for
// criterion rows; every other kind (a gate, a ratchet finding, a cliquet
// row) is Ack's domain (D11).
var ErrNotACriterion = errors.New("quality: sign only applies to criterion rows (use quality ack for this row)")

// ErrCriterionRequiresSign is returned by Ack when the targeted row's Kind
// DOES start with "criterion" — a criterion is ATTESTED, never absolved;
