...
//
// Design (SPEC-123 — native multi-agent parity):
...
//   - Project agent profiles are generated by mneme init under .codex/agents;
//     host install registers the same enforcement hooks Claude receives.
...
func Codex(binaryPath string) *Agent {
	return &Agent{
		...
		// Manual injects the Codex operating manual into ~/.codex/AGENTS.md
		...
		// Agents is nil because project profiles are generated by mneme init,
		...
		DelegationHook: func() (string, []HookPatch, error) {
			home, err := os.UserHomeDir()
			if err != nil {
				return "", nil, fmt.Errorf("install: codex: delegation hook: home dir: %w", err)
			}
			return filepath.Join(codexhome.Resolve(home), "hooks.json"), []HookPatch{
		...
		// Runtime-aware CLI/MCP management mirrors mutations to this directory.
		Skills: BundledSkillEntries,

		// AgentsDir is empty — the "Agent models" step is skipped for Codex.
		// Per-project model settings are rendered by mneme init.
		AgentsDir: "",
