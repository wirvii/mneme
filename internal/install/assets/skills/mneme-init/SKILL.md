...
8. On confirmation, call `subagent_write` with the same `role`/`archetype` and the (possibly user-edited) `composed_md`, plus `areas`, `areas_complete` (from 7b), and `engine` (defaults to `passthrough`).
